use crate::{
    error::CoreError, hash::sha256_hex_parts, merkle::calculate_merkle_root, pow::meets_difficulty,
    transaction::Transaction, CoreResult,
};
use chrono::Utc;
...
impl Block {
    ...
    pub fn validate_integrity(&self, difficulty: u32, skip_pow: bool) -> CoreResult<()> {
        ...
        if !skip_pow && !meets_difficulty(&self.hash, difficulty) {
...
mod tests {
    ...
    fn mined_block_should_pass_integrity_check() {
        ...
        let mut block = Block::new(1, vec![tx], "prev-hash");
        block.mine(1);

        assert!(block.validate_integrity(1, false).is_ok());
    }
}
