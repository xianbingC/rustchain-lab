...
fn handle_p2p_command(config: &AppConfig, args: &[String]) -> AppResult<()> {
    if args.len() < 2 {
        return Err(AppError::Command(
            "p2p 命令缺少子命令，可用: status/peers/sync-target/sync-plan/nearest-peers/dht-buckets/bootstrap/discover/diagnose/register-peer/ping/get-chain-status/find-node/chain-status/get-blocks/get-mempool"
                .to_string(),
        ));
    }
    ...
    match args[1].as_str() {
        ...
        "sync-plan" => {
            ...
            print_json("p2p_sync_plan", response)
        }
        "nearest-peers" => {
            ...
            if limit == 0 || limit > 128 {
                return Err(AppError::Command("limit 必须在 1~128 之间".to_string()));
            }
        ...
        "diagnose" => {
            ...
            let response = call_api_json(config, Method::POST, "/p2p/diagnose", Some(payload))?;
            print_json("p2p_diagnose", response)
        }
        "register-peer" => {
        ...
        "get-mempool" => {
            ...
            print_json("p2p_get_mempool", response)
        }
        "chain-status" => {
            let peer_id = require_arg(args, 2, "peer_id")?;
            let address = require_arg(args, 3, "address")?;
...
fn print_help() {
    ...
    println!("  rustchain-cli p2p diagnose [target_peer_id] [limit]");
